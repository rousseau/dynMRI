...
class DRIT(pl.LightningModule):
    ...
    def initialize(self):
        ...
        # self.discriminator_LR1.apply(gaussian_weights_init)
        self.discriminator_HR.apply(gaussian_weights_init)
        # self.discriminator_HR1.apply(gaussian_weights_init)
    ...
    def forward(self, x, y):
        if self.mode=='reconstruction':
            ...
            sys.exit("Préciser un mode valide pour l'inférence: reconstruction ou degradation")
    ...
    def infer_batch(self, batch):
        ...
        z_HR=self.encode_HR(y)
        z_random = self.get_z_random(x.shape[0], 8, 'gauss')
        ...
        return x, y, fake_LR, fake_HR, fake_LRLR, fake_HRHR, est_LR, est_HR, z_LR, z_HR, content_LR, content_HR, z_fake_LR, z_fake_HR, content_fake_LR, content_fake_HR, z_random, fake_LR_random, fake_HR_random, z_random_LR, z_random_HR



    def training_step(self, batch, batch_idx, optimizer_idx):
        x, y, fake_LR, fake_HR, fake_LRLR, fake_HRHR, est_LR, est_HR, z_LR, z_HR, content_LR, content_HR, z_fake_LR, z_fake_HR, content_fake_LR, content_fake_HR, z_random, fake_LR_random, fake_HR_random, z_random_LR, z_random_HR = self.infer_batch(batch)
        ...
        loss_G_LR, loss_G_HR=self.backward_EG(x, y, fake_LR, fake_HR, fake_LRLR, fake_HRHR, est_LR, est_HR, z_LR, z_HR, content_LR, content_HR, z_fake_LR, z_fake_HR, content_fake_LR, content_fake_HR)
        loss_z_LR, loss_z_HR=self.backward_G_alone(fake_LR_random, fake_HR_random, z_random_LR, z_random_HR, z_random)
        loss_D_content=self.backward_D_content(content_LR, content_HR)
        loss_D1_LR = self.backward_D_domain(self.discriminator_LR, x, fake_LR)
        loss_D1_HR = self.backward_D_domain(self.discriminator_HR, y, fake_HR)
        if optimizer_idx == 0: #encoder content
            self.log('train_loss_DRITpp_encoder_content', loss_G_LR+loss_G_HR+loss_z_LR+loss_z_HR, prog_bar=True)
            return (loss_G_LR+loss_G_HR+loss_z_LR+loss_z_HR)
    ...
    def validation_step(self, batch, batch_idx):
        x, y, fake_LR, fake_HR, fake_LRLR, fake_HRHR, est_LR, est_HR, z_LR, z_HR, content_LR, content_HR, z_fake_LR, z_fake_HR, content_fake_LR, content_fake_HR, z_random, fake_LR_random, fake_HR_random, z_random_LR, z_random_HR = self.infer_batch(batch) # y_hat, x, y, mask = self.infer_batch(batch)  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        plt.figure()
